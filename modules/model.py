...
class Material:
    ...
    # Børre Stenseth former former webpage
    def set_default_material(self, material: str = 'basic') -> None:
        ...
        # metals
            ...
            self._ambient_incidence = glm.vec3((0.329412, 0.223529, 0.027451))
            self._diffuse_incidence = glm.vec3((0.780392, 0.568627, 0.113725))
            self._specular_incidence = glm.vec3((0.992157, 0.941176, 0.807843))
        elif material == 'bronze':
            self._surface_brightness = 25.6
            self._ambient_incidence = glm.vec3((0.2125, 0.1275, 0.054))
            self._diffuse_incidence = glm.vec3((0.714, 0.4284, 0.18144))
            self._specular_incidence = glm.vec3((0.393548, 0.271906, 0.166721))
        elif material == 'polished_bronze':
            self._surface_brightness = 76.8
            self._ambient_incidence = glm.vec3((0.25, 0.148, 0.06475))
            self._diffuse_incidence = glm.vec3((0.4, 0.2368,  0.1036))
            self._specular_incidence = glm.vec3((0.774597, 0.458561, 0.200621))
        elif material == 'chrome':
            self._surface_brightness = 84.48
            self._ambient_incidence = glm.vec3((0.25, 0.25, 0.25))
            self._diffuse_incidence = glm.vec3((0.4, 0.4, 0.4))
            self._specular_incidence = glm.vec3((0.774597, 0.774597, 0.774597))
        elif material == 'copper':
            self._surface_brightness = 12.8
            self._ambient_incidence = glm.vec3((0.19125, 0.0735, 0.0225))
            self._diffuse_incidence = glm.vec3((0.7038, 0.27048, 0.0828))
            self._specular_incidence = glm.vec3((0.256777, 0.137622, 0.086014))
        elif material == 'polished_copper':
            self._surface_brightness = 51.2
            self._ambient_incidence = glm.vec3((0.2295, 0.08825, 0.0275))
            self._diffuse_incidence = glm.vec3((0.5508, 0.2118, 0.066))
            self._specular_incidence = glm.vec3((0.580594, 0.223257, 0.0695701))
        elif material == 'gold':
            self._surface_brightness = 51.2
            ...
            self._ambient_incidence = glm.vec3((0.24725, 0.2245, 0.0645))
            self._diffuse_incidence = glm.vec3((0.34615, 0.3143, 0.0903))
            self._specular_incidence = glm.vec3((0.797357, 0.723991, 0.208006))
        elif material == 'pewter':
            self._surface_brightness = 9.84615
            self._ambient_incidence = glm.vec3((0.105882, 0.058824, 0.113725))
            self._diffuse_incidence = glm.vec3((0.427451, 0.470588, 0.541176))
            self._specular_incidence = glm.vec3((0.333333, 0.333333, 0.521569))
        elif material == 'silver':
            self._surface_brightness = 51.2
            self._ambient_incidence = glm.vec3((0.19225, 0.19225, 0.19225))
            self._diffuse_incidence = glm.vec3((0.50754, 0.50754, 0.50754))
            self._specular_incidence = glm.vec3((0.508273, 0.508273, 0.508273))
        elif material == 'polished_silver':
            self._surface_brightness = 89.6
            self._ambient_incidence = glm.vec3((0.23125, 0.23125, 0.23125))
            self._diffuse_incidence = glm.vec3((0.2775, 0.2775, 0.2775))
            self._specular_incidence = glm.vec3((0.773911, 0.773911, 0.773911))
        # gems
            ...
            self._surface_brightness = 12.8
            self._ambient_incidence = glm.vec3((0.135, 0.2225, 0.1575))
            self._diffuse_incidence = glm.vec3((0.54, 0.89, 0.63))
            self._specular_incidence = glm.vec3((0.316228, 0.316228, 0.316228))
        elif material == 'obsidian':
            self._surface_brightness = 38.4
            self._ambient_incidence = glm.vec3((0.05375, 0.05, 0.06625))
            self._diffuse_incidence = glm.vec3((0.18275, 0.17, 0.22525))
            self._specular_incidence = glm.vec3((0.332741, 0.328634, 0.346435))
        elif material == 'pearl':
            self._surface_brightness = 113.664
            self._ambient_incidence = glm.vec3((0.25, 0.20725, 0.20725))
            self._diffuse_incidence = glm.vec3((1.0, 0.829, 0.829))
            self._specular_incidence = glm.vec3((0.296648, 0.296648, 0.296648))
        elif material == 'ruby':
            self._surface_brightness = 76.8
            self._ambient_incidence = glm.vec3((0.1745, 0.01175, 0.01175))
            self._diffuse_incidence = glm.vec3((0.61424, 0.04136, 0.04136))
            self._specular_incidence = glm.vec3((0.727811, 0.626959, 0.626959))
        elif material == 'turquoise':
            self._surface_brightness = 12.8
            self._ambient_incidence = glm.vec3((0.1, 0.18725, 0.1745))
            self._diffuse_incidence = glm.vec3((0.396, 0.74151, 0.69102))
            self._specular_incidence = glm.vec3((0.297254, 0.30829, 0.306678))
        # artificial
        elif material == 'black_plastic':
            self._surface_brightness = 32.0
            self._ambient_incidence = glm.vec3((0.0, 0.0, 0.0))
            self._diffuse_incidence = glm.vec3((0.01, 0.01, 0.01))
            self._specular_incidence = glm.vec3((0.5, 0.5, 0.5))
        elif material == 'cyan_plastic':
            self._surface_brightness = 32.0
            self._ambient_incidence = glm.vec3((0.0, 0.1, 0.06))
            self._diffuse_incidence = glm.vec3((0.0, 0.50980392, 0.50980392))
            self._specular_incidence = glm.vec3((0.50196078, 0.50196078, 0.50196078))
        elif material == 'green_plastic':
            self._surface_brightness = 32.0
            self._ambient_incidence = glm.vec3((0.0, 0.0, 0.0))
            self._diffuse_incidence = glm.vec3((0.1, 0.35, 0.1))
            self._specular_incidence = glm.vec3((0.45, 0.55, 0.45))
        elif material == 'red_plastic':
            self._surface_brightness = 32.0
            self._ambient_incidence = glm.vec3((0.0, 0.0, 0.0))
            self._diffuse_incidence = glm.vec3((0.5, 0.0, 0.0))
            self._specular_incidence = glm.vec3((0.7, 0.6, 0.6))
        elif material == 'white_plastic':
            self._surface_brightness = 32.0
            self._ambient_incidence = glm.vec3((0.0, 0.0, 0.0))
            self._diffuse_incidence = glm.vec3((0.55, 0.55, 0.55))
            self._specular_incidence = glm.vec3((0.7, 0.7, 0.7))
        elif material == 'yellow_plastic':
            self._surface_brightness = 32.0
            self._ambient_incidence = glm.vec3((0.0, 0.0, 0.0))
            self._diffuse_incidence = glm.vec3((0.5, 0.5, 0.0))
            self._specular_incidence = glm.vec3((0.6, 0.6, 0.5))
        elif material == 'black_rubber':
            self._surface_brightness = 10.0
            self._ambient_incidence = glm.vec3((0.02, 0.02, 0.02))
            self._diffuse_incidence = glm.vec3((0.01, 0.01, 0.01))
            self._specular_incidence = glm.vec3((0.4, 0.4, 0.4))
        elif material == 'cyan_rubber':
            self._surface_brightness = 10.0
            self._ambient_incidence = glm.vec3((0.02, 0.05, 0.05))
            self._diffuse_incidence = glm.vec3((0.4, 0.5, 0.5))
            self._specular_incidence = glm.vec3((0.04, 0.7, 0.7))
        elif material == 'green_rubber':
            self._surface_brightness = 10.0
            self._ambient_incidence = glm.vec3((0.0, 0.05, 0.0))
            self._diffuse_incidence = glm.vec3((0.4, 0.5, 0.4))
            self._specular_incidence = glm.vec3((0.04, 0.7, 0.04))
        elif material == 'red_rubber':
            self._surface_brightness = 10.0
            self._ambient_incidence = glm.vec3((0.05, 0.0, 0.0))
            self._diffuse_incidence = glm.vec3((0.5, 0.4, 0.4))
            self._specular_incidence = glm.vec3((0.7, 0.04, 0.04))
        elif material == 'white_rubber':
            self._surface_brightness = 10.0
            self._ambient_incidence = glm.vec3((0.05, 0.05, 0.05))
            self._diffuse_incidence = glm.vec3((0.5, 0.5, 0.5))
            self._specular_incidence = glm.vec3((0.7, 0.7, 0.7))
        elif material == 'yellow_rubber':
            self._surface_brightness = 10.0
            self._ambient_incidence = glm.vec3((0.05, 0.05, 0.0))
            self._diffuse_incidence = glm.vec3((0.5, 0.5, 0.4))
            self._specular_incidence = glm.vec3((0.7, 0.7, 0.04))
